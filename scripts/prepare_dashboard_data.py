...
def estimate_trade_size(row: pd.Series) -> float:
    trades = row.get("number_of_trades")
    deployed = row.get("total_deployed")
    if pd.notna(trades) and pd.notna(deployed) and trades and trades > 0:
        return float(deployed) / float(trades)
    return 100.0
...
def enrich_rolling_windows(df: pd.DataFrame) -> pd.DataFrame:
    if "average_net_profit_per_trade" not in df.columns:
        if {"net_profit", "number_of_trades"}.issubset(df.columns):
            df["average_net_profit_per_trade"] = (
                df["net_profit"] / df["number_of_trades"].replace({0: np.nan})
            )
        else:
            df["average_net_profit_per_trade"] = np.nan

    df["expected_profit_per_trade"] = df["average_net_profit_per_trade"]
    df["trade_size"] = df.apply(estimate_trade_size, axis=1)
    df["expected_return_per_trade_pct"] = df["expected_profit_per_trade"] / df["trade_size"]

    avg_wins: list[float | None] = []
    avg_losses: list[float | None] = []
    for _, row in df.iterrows():
        average_win, average_loss = expectancy_from_pf(
            avg_net=row.get("average_net_profit_per_trade"),
    ...
    df.drop(columns=["trade_size"], inplace=True)
    return df
