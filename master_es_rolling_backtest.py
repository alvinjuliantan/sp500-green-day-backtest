...
def prepare_signals(data):
    ...
    # Your exact strategy:
    # Red -> Green closes -> buy next session open -> sell next session close
    signals["buy_after_first_green_after_red"] = (
        df["green"] & df["red"].shift(1)
    ).shift(1)

    # If today closes green, buy next session
    signals["buy_after_every_green"] = (
        df["green"]
    ).shift(1)

    # If today closes red, buy next session
    signals["buy_after_red"] = (
        df["red"]
    ).shift(1)

    # Red -> Red -> Green closes -> buy next session
    signals["buy_after_first_green_after_two_reds"] = (
        df["green"] & df["red"].shift(1) & df["red"].shift(2)
    ).shift(1)

    # Red -> Green closes above 200DMA -> buy next session
    ...
    # Red -> Green closes below 200DMA -> buy next session
    ...
    # Benchmark: buy every session open-to-close
...
def create_interpretation(rolling_summary):
    if rolling_summary.empty:
        return "No strategy data available."

    top = rolling_summary.iloc[0]
    main_rows = rolling_summary[
        rolling_summary["strategy"] == "buy_after_first_green_after_red"
    ]

    lines = []

    lines.append(
        f"**Top-ranked strategy:** `{top['strategy']}` was profitable in "
        f"**{top['probability_profitable_1yr']:.2%}** of rolling one-year windows."
    )
    ...
    if not main_rows.empty:
        ...
        lines.append(
            f"**Main strategy:** `buy_after_first_green_after_red` was profitable in "
            f"**{main['probability_profitable_1yr']:.2%}** of rolling one-year windows, "
            f"with median one-year net P/L of **{format_currency(main['median_1yr_net_profit'])}**."
        )

        if main["probability_profitable_1yr"] >= 0.65 and main["median_1yr_net_profit"] > 0:
            verdict = "This looks promising on this approximate Yahoo dataset, but still needs validation with proper CME session data."
        elif main["probability_profitable_1yr"] >= 0.55 and main["median_1yr_net_profit"] > 0:
            verdict = "This is mildly positive but not strong enough yet. Treat it as inconclusive."
        else:
            verdict = "This looks weak under the current Yahoo daily-data approximation."

        lines.append(f"**Direct verdict:** {verdict}")

    lines.append(
        "**Important warning:** This uses Yahoo daily `ES=F` data. Before trading real money, validate with proper CME intraday/session data."
    )
...
st.markdown(create_interpretation(rolling_summary))
...
with c2:
    st.subheader("Median one-year net P/L")
    ...
    fig_median = px.bar(
        ...
        orientation="h",
        labels={
            "median_1yr_net_profit": "Median one-year net P/L",
            "strategy_clean": "Strategy",
        },
    )
...
st.subheader("Rolling one-year net P/L over time")
...
st.caption(
    "KNNBCCB™ S&P Futures Edge Dashboard. Research tool only, not financial advice. Validate all signals with proper CME session data before trading real money."
)